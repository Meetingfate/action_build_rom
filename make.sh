...
do
  if [ $i = vendor ] || [ $i = mi_ext ];then
    ...
    Start_Time
    info=$("$workfile"/tools/gettype -i "$GITHUB_WORKSPACE"/Extra_dir/$i.img)
    if [ "$info" == "ext" ]; then
...
do
  if [[ "${Readaw}" == "true" ]] && [[ $i = mi_ext ]];then
    ...
    argvs+="--partition "$i":readonly:"$img_size":qti_dynamic_partitions --image "$i"="$GITHUB_WORKSPACE"/images/${i}.img "
