...
do
  if [[ "${Readaw}" == "true" ]] && [[ $i = mi_ext ]];then
    ...
    argvs+="--partition "$i"_a:none:$img_size:qti_dynamic_partitions_a --image "$i"_a="$GITHUB_WORKSPACE"/images/${i}_a.img --partition "$i"_b:none:0:qti_dynamic_partitions_b --image "$i"_b="$GITHUB_WORKSPACE"/images/${i}_b.img "
